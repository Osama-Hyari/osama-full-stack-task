...
function SummaryCard({
  ...
  return (
    ...
    >
      ...
      <Typography variant="h4" fontWeight={700} mb={1} color={color}>
        {value}
      </Typography>
      ...
      {chipLabel && (
        <Chip label={chipLabel} color={chipColor} size="small" sx={{ mt: 1 }} />
      )}
    </Paper>
  );
}
      ...
      {loading ? (
        ...
        <>
          ...
          <Stack direction="row" alignItems="center" spacing={2} mb={2}>
            ...
            <Chip
              label={`Most Common: ${data.mostCommonCategory}`}
              size="small"
              color="info"
              icon={<CategoryIcon />}
            />
  ...
  // The correct prop for MUI Grid is 'item', ensure it is used correctly. */}
          ...
          <Grid container spacing={3} mb={1}>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}>
              <SummaryCard
                ...
                description="Total income in range"
                color="success.main"
                chipLabel={`${data.incomePercent.toFixed(2)}%`}
                chipColor="success"
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}>
              <SummaryCard
                ...
                description="Total expense in range"
                color="error.main"
                chipLabel={`${data.expensePercent.toFixed(2)}%`}
                chipColor="error"
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 3 }}>
              <SummaryCard
                ...
                chipLabel={`Avg: ${formatCurrency(data.average)}`}
                chipColor="info"
              />
            </Grid>
          </Grid>
          ...
          <Grid container spacing={3} mb={1}>
            <Grid size={{ xs: 12, sm: 6, md: 6 }}>
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  {data.largest?.type} - {data.largest?.category} on{" "}
                  {formatDate(data.largest?.date)}
                </Typography>
              </Paper>
            </Grid>
            <Grid size={{ xs: 12, sm: 6, md: 6 }}>
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  {data.smallest?.type} - {data.smallest?.category} on{" "}
                  {formatDate(data.smallest?.date)}
                </Typography>
              </Paper>
            </Grid>
          </Grid>
          ...
          <Paper elevation={0} sx={{ p: 3, borderRadius: 3, mb: 2 }}>
            ...
            <Stack direction="row" spacing={2}>
              <Chip
                icon={<PaidIcon />}
                label={`Income: ${formatCurrency(data.typeBreakdown?.income)}`}
                color="success"
              />
              <Chip
                icon={<ReceiptLongIcon />}
                label={`Expense: ${formatCurrency(data.typeBreakdown?.expense)}`}
                color="error"
              />
            </Stack>
          </Paper>
